...
export default async function AdminDashboard() {
  const [currentGuests, todayPayments, subscriberCount] = await Promise.all([
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {session.profile.pictureUrl && (
                          ...
                          <img
                            src={session.profile.pictureUrl}
                            alt=""
                            className="w-8 h-8 rounded-full"
                          />
                        )}
                        <span className="font-medium">
                          {session.profile.displayName ?? '不明'}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-500">
                      {new Date(session.checkedInAt).toLocaleTimeString(
                        'ja-JP',
